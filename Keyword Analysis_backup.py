...
def load_model():
    ...
    model.load_state_dict(
        torch.load("C:\\github\\test2\\model_state_dict.pt", map_location=device), strict=False)
    return model
...
if search_word and start_date and end_date != "0":
    ...
    # 특정 키워드 기사 index 추출(제목에 포함 or 본문에 2번 이상 포함)
    target_index_list = []
    for i in range(0, len(date_data)):
        title_i = date_data.iloc[i, 2]
        body_i = date_data.iloc[i, 4]
        if title_i.count(search_word) > 0 or body_i.count(search_word) > 1:
